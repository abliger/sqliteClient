  ...
  <div class="h-full p-4 overflow-auto space-y-4">
    ...
    >
      ...
      <div class="bg-surface-50 rounded-lg p-4 space-y-3">
        ...
        <!-- 详细时间分解 -->
        <div v-if="executionInfo?.plan_time_ms || executionInfo?.query_time_ms" class="pl-6 space-y-1 text-sm text-surface-600">
        ...
        <!-- 返回行数 -->
        <div v-if="result.type === 'rows'" class="flex items-center space-x-2 text-surface-700">
          <TableCellsIcon class="w-4 h-4 text-surface-500" />
          <span class="font-medium">返回行数:</span>
          <span>{{ result.rows.length }} 行</span>
          <span v-if="result.has_more" class="text-amber-600 text-sm">(已截断)</span>
        </div>

        <!-- 扫描行数 -->
        <div v-if="executionInfo?.rows_scanned" class="flex items-center space-x-2 text-surface-700">
          <ListBulletIcon class="w-4 h-4 text-surface-500" />
        ...
        <template v-if="result.type === 'execution'">
          <div class="flex items-center space-x-2 text-surface-700">
            <BoltIcon class="w-4 h-4 text-surface-500" />
            <span class="font-medium">影响行数:</span>
            <span>{{ result.rows_affected }}</span>
          </div>
          <div
            v-if="result.last_insert_id"
            class="flex items-center space-x-2 text-surface-700"
          >
        ...
        <div v-if="executionInfo?.used_index !== undefined" class="flex items-center space-x-2">
          <InformationCircleIcon class="w-4 h-4 text-surface-500" />
          <span class="font-medium">索引使用:</span>
          <span v-if="executionInfo.used_index" class="text-green-600 flex items-center space-x-1">
            <CheckCircleIcon class="w-4 h-4" />
            <span>使用了索引</span>
            <span v-if="executionInfo.indexes_used.length > 0" class="text-surface-600">
              ({{ executionInfo.indexes_used.join(', ') }})
            </span>
          </span>
          <span v-else class="text-amber-600 flex items-center space-x-1">
        ...
        <!-- 全表扫描警告 -->
        <div v-if="executionInfo?.is_full_table_scan" class="flex items-center space-x-2 text-amber-600">
          <ExclamationTriangleIcon class="w-4 h-4" />
      ...
      <div v-if="hasWarnings" class="space-y-2">
        <h4 class="font-medium text-surface-700 flex items-center space-x-2">
          <ExclamationTriangleIcon class="w-4 h-4" />
          <span>警告</span>
        </h4>
        <div
        ...
        >
          <div class="flex items-start space-x-2">
            <component :is="warningIcon(warning.level)" class="w-4 h-4 mt-0.5 flex-shrink-0" />
            <div>
              <div class="font-medium">{{ warning.message }}</div>
              <div v-if="warning.code" class="text-xs opacity-75 mt-1">代码: {{ warning.code }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="hasSuggestions" class="space-y-2">
        ...
        <ul class="space-y-1">
          <li
            ...
            :key="index"
            class="flex items-start space-x-2 text-sm text-surface-600 bg-blue-50 p-2 rounded"
          >
            <LightBulbIcon class="w-4 h-4 text-blue-500 mt-0.5 flex-shrink-0" />
