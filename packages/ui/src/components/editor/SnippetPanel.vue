...
<template>
  <Transition appear enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 translate-x-full"
    ...
    <div
      class="fixed inset-y-0 right-0 w-80 bg-white dark:bg-surface-800 shadow-xl border-l border-surface-200 dark:border-surface-700 z-50 flex flex-col">
      <!-- 头部 -->
      <div
        ...
        <button class="p-1.5 rounded hover:bg-surface-200 dark:hover:bg-surface-700 text-surface-500"
          @click="emit('close')">
      ...
      <div class="flex border-b border-surface-200 dark:border-surface-700">
        <button class="flex-1 px-4 py-2 text-sm font-medium border-b-2 transition-colors" :class="activeTab === 'current'
          ? 'border-primary-500 text-primary-600 dark:text-primary-400'
          : 'border-transparent text-surface-500 hover:text-surface-700'" @click="activeTab = 'current'">
          <CircleStackIcon class="w-4 h-4 inline mr-1" />
          当前数据库
          <span class="ml-1 text-xs text-surface-400">({{ currentConnectionSnippets.length }})</span>
        </button>
        <button class="flex-1 px-4 py-2 text-sm font-medium border-b-2 transition-colors" :class="activeTab === 'global'
          ? 'border-primary-500 text-primary-600 dark:text-primary-400'
          : 'border-transparent text-surface-500 hover:text-surface-700'" @click="activeTab = 'global'">
      ...
      <div class="flex-1 overflow-y-auto px-4 py-2 space-y-2">
        ...
        <template v-if="activeTab === 'current'">
          ...
          <div v-for="snippet in currentConnectionSnippets" :key="snippet.id"
            ...
            <div class="flex items-start justify-between mb-1">
              ...
              <div
                class="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none group-hover:pointer-events-auto">
                <button class="p-1 text-surface-400 hover:text-primary-500 rounded" title="编辑"
                  @click.stop="openEditDialog(snippet)">
                  <PencilIcon class="w-3.5 h-3.5" />
                </button>
                <button class="p-1 text-surface-400 hover:text-danger-500 rounded" title="删除"
            ...
            <pre
              class="text-xs text-surface-600 dark:text-surface-400 bg-surface-50 dark:bg-surface-900/50 p-2 rounded overflow-hidden line-clamp-2 font-mono"><code><template v-for="(part, idx) in parseSqlPreview(snippet.sql)" :key="idx"><span v-if="part.type === 'variable'" class="text-primary-500 font-medium">{{ '{' + '{' + part.content + '}' + '}' }}</span><template v-else>{{ part.content }}</template></template></code>
</pre>
        ...
        <!-- 全局片段 -->
        <template v-else>
          <div v-for="snippet in globalSnippets" :key="snippet.id"
            class="group p-3 bg-white dark:bg-surface-800 rounded-lg border border-surface-200 dark:border-surface-700 hover:border-primary-300 dark:hover:border-primary-700 hover:shadow-sm transition-all cursor-pointer"
            :class="snippet.type === 'builtin' ? 'bg-surface-50/50' : ''" @click="useSnippet(snippet)">
            <div class="flex items-start justify-between mb-1">
              <div class="flex items-center min-w-0">
                <span class="text-sm font-medium text-surface-900 dark:text-surface-100 truncate">
                  {{ snippet.name }}
                </span>
                <span v-if="snippet.type === 'builtin'"
              ...
              <div
                class="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none group-hover:pointer-events-auto">
                <button v-if="snippet.type === 'custom'" class="p-1 text-surface-400 hover:text-primary-500 rounded"
                  title="编辑" @click.stop="openEditDialog(snippet)">
                  <PencilIcon class="w-3.5 h-3.5" />
                </button>
                <button v-if="snippet.type === 'custom'" class="p-1 text-surface-400 hover:text-danger-500 rounded"
                  title="删除" @click.stop="handleDelete(snippet)">
                  <TrashIcon class="w-3.5 h-3.5" />
                </button>
              </div>
            </div>
      ...
      <div v-if="showCreateDialog || showEditDialog"
        ...
        <div class="w-full max-w-2xl mx-4 bg-white dark:bg-surface-800 rounded-xl shadow-xl overflow-hidden">
          <div class="flex items-center justify-between px-6 py-4 border-b border-surface-200 dark:border-surface-700">
            ...
            <button class="p-1 rounded hover:bg-surface-100 dark:hover:bg-surface-700"
          ...
          <div class="px-6 py-4 space-y-4">
            ...
            <div>
              <label class="flex items-center space-x-2 cursor-pointer">
                <input v-model="formData.isGlobal" type="checkbox"
            ...
            <div>
              ...
              <textarea v-model="formData.sql" rows="6"
          ...
          <div
            class="flex justify-end items-center gap-3 px-6 py-4 border-t border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-900/50">
            <button
            ...
            <button
              ...
              :disabled="!formData.name || !formData.sql" @click="showCreateDialog ? handleCreate() : handleUpdate()">
      ...
      <!-- 变量填充对话框 -->
      <div v-if="showVariableDialog && selectedSnippet"
        class="fixed inset-0 z-50 flex items-center justify-center bg-black/50"
        ...
        <div class="w-full max-w-lg mx-4 bg-white dark:bg-surface-800 rounded-xl shadow-xl overflow-hidden">
          ...
          <div class="px-6 py-4 space-y-4">
            ...
            <div v-for="variable in (selectedSnippet.variables || extractVariables(selectedSnippet.sql))"
              :key="variable.name" class="space-y-1">
              <label class="block text-sm font-medium text-surface-700 dark:text-surface-300">
                {{ variable.name }}
                <span v-if="variable.required" class="text-danger-500">*</span>
              </label>
              <input v-model="variableValues[variable.name]" type="text"
                class="w-full px-3 py-2 rounded-lg border border-surface-300 dark:border-surface-600 bg-white dark:bg-surface-800 text-surface-900 dark:text-surface-100 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
            ...
            <div class="pt-2">
              ...
              <pre
                class="text-xs bg-surface-100 dark:bg-surface-900 p-3 rounded overflow-x-auto font-mono text-surface-700 dark:text-surface-300">
          <code>{{ fillTemplate(selectedSnippet.sql, variableValues) }}</code></pre>
            </div>
          </div>
          <div
            class="flex justify-end items-center gap-3 px-6 py-4 border-t border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-900/50">
