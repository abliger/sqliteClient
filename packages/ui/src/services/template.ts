...
export function filterSnippets(snippets: Snippet[], filter: SnippetFilter): Snippet[] {
    return snippets.filter(snippet => {
        // 按类型过滤
        if (filter.type && snippet.type !== filter.type) {
            return false
        }
        ...
        // 按标签过滤
        if (filter.tag && !snippet.tags.includes(filter.tag)) {
            return false
        }
        ...
        if (filter.keyword) {
            ...
            const matchesName = snippet.name.toLowerCase().includes(keyword)
            const matchesDesc = snippet.description?.toLowerCase().includes(keyword)
            const matchesSql = snippet.sql.toLowerCase().includes(keyword)
            const matchesTags = snippet.tags.some(t => t.toLowerCase().includes(keyword))

            if (!matchesName && !matchesDesc && !matchesSql && !matchesTags) {
                return false
            }
        }
 ...
 * 验证创建片段请求
 */
export function validateCreateRequest(request: CreateSnippetRequest): {
    valid: boolean
    error?: string
} {
 ...
 * 验证更新片段请求
 */
export function validateUpdateRequest(request: UpdateSnippetRequest): {
    valid: boolean
    error?: string
} {
