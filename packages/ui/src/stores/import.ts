...
export const useImportStore = defineStore('import', () => {
  ...
  const canProceed = computed(() => {
    switch (currentStep.value) {
      ...
      case 'mapping':
        return config.value.table_name.trim().length > 0 && 
  ...
  async function parseFile() {
    ...
    try {
      ...
      if (preview.value) {
        ...
        config.value.column_mappings = generateColumnMappings(preview.value, config.value.table_name)
      }
    } catch (err) {
  ...
  function nextStep() {
    const steps: ('file' | 'mapping' | 'preview' | 'importing')[] = ['file', 'mapping', 'preview']
