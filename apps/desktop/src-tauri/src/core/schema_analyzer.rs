...
use crate::models::schema::{
    ColumnInfo, DatabaseSchema, ForeignKeyInfo, IndexInfo, TableInfo, TriggerInfo,
};
...
impl SchemaAnalyzer {
    pub fn get_database_schema(pool: &Pool<SqliteConnectionManager>) -> AppResult<DatabaseSchema> {
        let _conn = pool
            .get()
    ...
    pub fn generate_er_diagram_data(pool: &Pool<SqliteConnectionManager>) -> AppResult<ERDiagram> {
        let tables = Self::list_tables(pool)?;
